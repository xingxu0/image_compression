...
def create_table(comp, dep1_s, dep2_s):
	...
	co = {}   #co for code
	oc = {}   #oc for occur, then becomes differenceprint get_bits(abs((int(s[j]) >> 1) - last_dc)), get_bits(abs(int(s[j]) - last_dc_t))
	pos = 1
		...
		for ii in range(len(block)):
			...
			for i in range(1, 64):
				...
				while (r > 15):
					...
					if pos <= 5:
						poss = 1
					lib.record_code(block, block_o, ii, 0xf0, poss, pos + 15, oc)
					pos += 16
					r -= 16
				...
				if pos <= 5:
					poss = 1
				lib.record_code(block, block_o, ii, (r << 4) + b[i], poss, i, oc)
				pos = i + 1
				r = 0
			if r > 0:
				poss = 2
				if pos <= 5:
					poss = 1
				lib.record_code(block, block_o, ii, 0, poss, 63, oc)
