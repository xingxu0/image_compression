...
def create_table(comp, dep1_s, dep2_s):
	...
	for f in files:
		...
		block_o = lib.get_blocks_with_dc_in_diff_with_threshold(f, comp)	
		for ii in range(len(block)):
			...
			b = block[ii]
			b_o = block_o[ii]
			oc_dc[lib.get_previous_block(block_o, ii) [0]][b_o[0]] += 1
			oc_dc_old[lib.get_previous_block(block, ii) [0]][b[0]] += 1
		
	lib.fprint("generating DC tables...")
	for i in range(12):
		if comp == '0':
			co_dc[i] = lib.huff_encode_plus_extra(oc_dc[i], lib.bits_dc_luminance)
			co_dc_old[i] = lib.huff_encode_plus_extra(oc_dc_old[i], lib.bits_dc_luminance)
		else:
			co_dc[i] = lib.huff_encode_plus_extra(oc_dc[i], lib.bits_dc_chrominance)
			co_dc_old[i] = lib.huff_encode_plus_extra(oc_dc_old[i], lib.bits_dc_chrominance)
		save_code_table(co_dc[i], oc_dc[i], "DC", i, "", table_folder)
		save_code_table(co_dc_old[i], oc_dc_old[i], "DC_old", i, "", table_folder)
