...
def create_table(comp, dep1_s, dep2_s):
	...
	for i in range(12):
		if comp == '0':
			co_dc[i] = lib.huff_encode_entropy_plus_extra(oc_dc[i], lib.bits_dc_luminance)
		else:
			co_dc[i] = lib.huff_encode_entropy_plus_extra(oc_dc[i], lib.bits_dc_chrominance)
	...
	for i in range(1, 64):
		...
		for p in range(SIZE1 + 1):
			for pp in range(SIZE2 + 1):
				co[i][p][pp] = lib.huff_encode_entropy_plus_extra(oc[i][p][pp], lib.code)
