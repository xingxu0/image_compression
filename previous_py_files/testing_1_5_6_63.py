		...
		for ii in range(len(block_t)):
			...
			for i in range(1, 64):
				...
				while (r > 15):
					...
					if pos <= 5:
						poss = 1
					lib.record_jpeg(block_t, block_t_o, ii, 0xf0, poss, pos + 15, jpeg_t)
					a1,a2,a3,a4=lib.record_code(block_t, block_t_o, ii, 0xf0, poss, pos + 15, oc_t)
					oc_opt[0xf0] += 1
					pos += 16
					r -= 16
				...
				if pos <= 5:
					poss = 1
				a1,a2,a3,a4=lib.record_code(block_t, block_t_o, ii, (r << 4) + b[i], poss, i, oc_t)
				if not (((r<<4) + b[i]) in co[a1][a2][a3]):
					print (r<<4) + b[i], co[a1][a2][a3]
				gp_2 += lib.code[(r<<4)+b[i]] - co[a1][a2][a3][(r<<4)+b[i]]
				lib.record_jpeg(block_t, block_t_o, ii, (r << 4) + b[i], poss, i, jpeg_t)
				oc_opt[(r << 4) + b[i]] += 1
				pos = i + 1
				r = 0
			if r > 0:
				...
				if pos <= 5:
					poss = 1	
				lib.record_jpeg(block_t, block_t_o, ii, 0, poss, 63, jpeg_t)
				a1,a2,a3,a4=lib.record_code(block_t, block_t_o, ii, 0, poss, 63, oc_t)
