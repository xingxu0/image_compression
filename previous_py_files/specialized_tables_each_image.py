...
def create_table(filename, comp, dep1_s, dep2_s):
	global tab_folder
	...
	ratio += 1
	sys.stdout.flush()
	block = lib.get_blocks_all_in_bits(filename, comp)
	block_o = lib.get_blocks_with_dc_in_diff(filename, comp)			
...
for f in files:
	...
	t_opt = 0
	for c in range(3):
		t_g, t_t, t_o = create_table(f, str(c), dep1, dep2)
		g += t_g
		t += t_t
		t_opt += t_o
		total_g += t_g
		...
		total_opt += t_o
	print f, ":",
	print "baseline bits:", str(t), 
	print "gaining " + str(g) + " bits (" + str(g*100.0/t)+"%)",
	print "optimize bits:", str(t_opt),
	print "gaining " + str(g+t_opt-t) + " bits (" + str((g+t_opt-t)*100.0/t_opt)+"%)"

print "finally"
print "baseline bits:", str(total_t), 
print "gaining " + str(total_g) + " bits (" + str(total_g*100.0/total_t)+"%)",
print "optimize bits:", str(total_opt),
print "gaining " + str(total_g+total_opt-total_t) + " bits (" + str((total_g+total_opt-total_t)*100.0/total_opt)+"%)"
