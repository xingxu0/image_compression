...
def calc_gain(comp, dep1_s, dep2_s):
	...
	for f in files:
		...
		for ii in range(len(block_t)):
			total_count += 1
			if (bigger and block_t[ii][0] <= thre) or (not bigger and block_t[ii][0] >= thre):
				total_throw += 1
				continue
	...
	lib.fprint("\nTEST  SET:")
	lib.fprint("in total " + str(total_count) + ", throw away " + str(total_throw) + " ( " + str(total_throw*1.0/total_count) +" ) ")
...
dep1, dep2, bigger, thre = lib.get_deps_from_file_partial(tab_folder + "/dep.txt")
bigger = (bigger == "G" or bigger == "g")
thre = int(thre)
lib.index_file = out_file

lib.fprint(str(dep1) + " " + str(dep2) + " " + str(bigger) + " " + str(thre))
g = 0
t = 0
t_opt = 0
for c in range(3):
