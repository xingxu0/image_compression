...
def generate_blocks(folder):
	...
	for f in f_jpgs:
		name = f[f.rfind("/") + 1 : ]
		sys.stdout.flush()
		cmd = "/opt/libjpeg-turbo/bin/jpegtran -outputcoef temp__.block " + f + " temp__.out"
		os.system(cmd)
		os.system("python lossy_zerooff.py temp__.block " + folder + "/" + name + ".block " + str(zerooff_thre) + " 75")
	os.system("rm temp__.out")
	os.system("rm temp__.block")
