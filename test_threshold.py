...
for f in folders:
	...
	for i in range(1, 101):#101):
		c = commands.getstatusoutput("/opt/libjpeg-turbo/bin/jpegtran -outputcoef " + str(i) + "_temp.block " +  "images/" + f + "_Q75/" + str(i) + ".jpg temp.jpg")
		c = commands.getstatusoutput("python lossy_zerooff.py " + str(i) + "_temp.block py.block 0.003 75")
		...
		c = commands.getstatusoutput("/opt/libjpeg-turbo/bin/jpegtran -outputcoef libjpeg.block " + str(i) + "_out.jpg temp.jpg")
		c = commands.getstatusoutput("diff libjpeg.block py.block")
		os.system("diff libjpeg.block py.block")
		if (c[1] != ""):
			print c[1]
	if first_seen == False:
