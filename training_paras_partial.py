...
def create_table(comp, dep1_s, dep2_s):
	...
	for f in files:
		...
		for ii in range(len(block)):			
			if (bigger and block[ii][0] <= thre) or (not bigger and block[ii][0] >= thre):
				continue
	...
	for i in range(1, 64):
		...
		for p in range(SIZE1 + 1):
			for pp in range(SIZE2 + 1):
				co[i][p][pp] = lib.huff_encode_plus_extra(oc[i][p][pp], code)
				save_code_table(co[i][p][pp], oc[i][p][pp], i, p, pp, table_folder)
	lib.index_file.close()
	print "\n\tTraining DONE"
	
if len(sys.argv) < 5:
	print "usage: python training.py [TRAINING IMAGE FOLDER] [TABLE FOLDER] [DEPENDANT 1] [DEPENDANT 2] [G or S] [THRESHOLD] (if it's G, keep blocks > threshold.)"
...
dep1 = sys.argv[3]
dep2 = sys.argv[4]
bigger = (sys.argv[5] == "G" or sys.argv[5] == "g")
thre = int(sys.argv[6])


dep_file = open(tab_folder + "/dep.txt", "w")
dep_file.write(dep1 + " " + dep2 + " " + sys.argv[5] + " " + str(thre) + "\n")
dep_file.close()
