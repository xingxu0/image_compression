	...
	for f in files:
		...
		for ii in range(len(block)):
			...
			for i in range(1, 64):
				...
				while (r > 15):
					lib.record_code(block, block_o, ii, 0xf0, pos, pos + 15, oc, 0, 0, 0)
					pos += 16
					r -= 16

				lib.record_code(block, block_o, ii, (r << 4) + b[i], pos, i, oc, 0, 0, 0)
				pos = i + 1
				r = 0
			if r > 0:
				lib.record_code(block, block_o, ii, 0, pos, 63, oc, 0, 0, 0)
