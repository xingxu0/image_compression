...
def zero_off(b, b_o, code, ci, sf):
	...
	while i + 1 < len(r_l):
		...
		#if diff >= thre and psnr + pow((quant[ci][jpeg_natural[p[i]]]*sf+50)/100, 2)<threshold:
		if diff >= thre and (1+modified)*1.0/total_non_zero<=threshold:
#*pow(quant[ci][jpeg_natural[p[i]]], 2):
		#if diff >= thre:
			if modified == 0:
				#print " "
				#print " "
