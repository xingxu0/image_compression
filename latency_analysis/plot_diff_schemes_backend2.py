...
#ax.set_xscale("log")
#ax.set_yscale("log")
ax.set_title("entire_fetching_latency")

savefig("plot_schemes_backend.png")
...
for j in range(len(hr)):
	...
	ax.grid()
	if j > 0:	
		savefig("plot_schemes_backend_perce_%s.png"%(schemes[j-1][0]))
	else:
		savefig("plot_schemes_backend_perce_%s.png"%("FB"))
