...
for i in range(len(hr)):
	...
	if i > 0:
		if t_schemes[i][0] == "L-ROMP-Same-Size":
			ax.plot(x[:t__], cdf_y[:t__], "--b", linewidth = line_width)
		else:
			ax.plot(x[:t__], cdf_y[:t__], "--b", linewidth = line_width)
	else:
...
savefig("plot_backend_only_.png")#%(tested_file_size))
savefig("plot_backend_only_.eps")#%(tested_file_size))
