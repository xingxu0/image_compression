...
for ss in t_schemes:
	...
	for f in fname:
		if os.path.isfile(f):
			continue
		s = f[:f.find("_")]
		os.system("python estimate_latency.py %s_1st.obj_pdf %s_xfer.obj_pdf %s_%d.obj_pdf_gen_%s %f"%(s, s, s, tested_file_size, str(ss[3]),l_s* tested_file_size*1.0/measured_file_size))
...
for i in range(len(hr)):
	...
	if i > 0:
		if t_schemes[i][0] == "L-ROMP-Same-Size":
			...
			lll1, = ax.plot(x[:t__], cdf_y[:t__], "--b", linewidth=line_width)
	else:
		lll0, = ax.plot(x[:t__], cdf_y[:t__], "-k", linewidth = line_width)
	x_all[i] = x
	y_all[i] = cdf_y
	'''
	if i == 1:
...
#ax2.legend(legend2)
ax2.set_xlabel("Difference", fontsize = label_font_size)
ax2.grid()
#ax2.set_yscale("log")
...
#ax.legend(legend, 4)
print lll
#lgd = fig.legend((lll0,lll1),("W/o Re-Compression", "W/ Re-Compression"), ncol=2,  fontsize=legend_font_size, loc='upper center', bbox_to_anchor=(0.5, 1.08))
...
savefig("plot_edge_fb.png",)# bbox_extra_artists=(lgd,), bbox_inches='tight')#%(tested_file_size))
savefig("plot_edge_fb.eps",)# bbox_extra_artists=(lgd,), bbox_inches='tight')#%(tested_file_size))
