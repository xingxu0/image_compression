...
{
	char filename[200];
	sprintf(filename, "%s/plain_coef_bins_%d", table_folder, c);
	FILE * f = fopen(filename, "r");
	if (f == NULL) return;
...
/* old simpler approach
	for (int i = 0; i < htbl->length; i++)
	{
		//	printf("ls table value %d", ls_table[i].value);
		if (htbl->symbol[i] == code)
			return i;
	}
	*/
}
...
int count = 0;
METHODDEF(boolean)
decode_mcu_entropy (j_decompress_ptr cinfo, JBLOCKROW *MCU_data)
{
  huff_entropy_ptr entropy = (huff_entropy_ptr) cinfo->entropy;
  int blkn;
  BITREAD_STATE_VARS;
  ...
  if (! entropy->insufficient_data) {
    ...
    for (blkn = 0; blkn < cinfo->blocks_in_MCU; blkn++) {
      ...
      /* Section F.2.2.1: decode the DC coefficient difference */
      p_table = &dc_table[ci][state.last_dc_diff[ci]];
...
#ifdef DEBUG
      printf("ci %d dc bits(%d): %d (%d/%d)\t", ci, state.last_dc_diff[ci], s, -1, -1);
#endif
      	...
      	(*block)[0] = (JCOEF) s;
      		...
      		//s = jpeg_huff_decode_entropy(&br_state, get_buffer, bits_left, p_table, 1);
					...
					if (s) {
						...
						if (r != 15)
							goto EndOfBlock;
						for (int temp_i=k; temp_i<=k + 15; ++temp_i)
							ma += max_pos_value[ci].bits[temp_i];
						k += 15;
					}
				}
...
start_pass_huff_decoder (j_decompress_ptr cinfo)
{
  ...
  if (cinfo->progressive_mode) {
    ...
    if (cinfo->lim_Se != DCTSIZE2-1)
      ...
      entropy->pub.decode_mcu = cinfo->private_option == 2 ? decode_mcu_entropy : decode_mcu; // Xing
