...
boolean initialize_AC_table(int c, int i, int j, int k)
{
	...
	*/
	...
	for (ii = 0; ii < table_size; ++ii)
	{
		fscanf(f, "%d: %d", &(ac_table[c][i][j][k].bits[ii]), &(ac_table[c][i][j][k].run_length[ii]));
...
void initialize_DC_table(int c, int i)
{
	...
	for (ii=0; ii<12; ++ii)
	{
		fscanf(f, "%d: %d", &(dc_table[c][i].bits[ii]), &(dc_table[c][i].run_length[ii]));
  ...
  for (blkn = 0; blkn < cinfo->blocks_in_MCU; blkn++) {
    ...
    int real_last_non_zero = 1;
      ...
      for (k = 1; k < DCTSIZE2; k++) {
    	  ...
    	  p_table = &ac_table[ci][k][get_first_dimension_index(ci, k, f, tmp)][get_second_dimension_index(ci, k, &state.previous_block_state, tmp)];
