      ...
      int ci, int last_dc_diff_bits, int * dc_diff_bits, previous_block_state_t * previous_block_state)
{
  ...
  code = p_table->symbol[nbits];
  size = p_table->bits[nbits];
  PUT_BITS(code, size)
  CHECKBUF15()
  ...
  #define kloop_entropy(k, jpeg_natural_order_of_k) {  \
    if ((temp = block[jpeg_natural_order_of_k]) == 0) { \
      ...
      while (r > 15) { \
        f = t == 0 ? 0 : t*1000/ma; \
        p_table = &ac_table[ci][last_non_zero][get_first_dimension_index(ci, last_non_zero, f, *dc_diff_bits)][get_second_dimension_index(ci, last_non_zero, previous_block_state, *dc_diff_bits)]; \
