...
for q in range(1, 101):
	...
	for f in fs:
		i += 1
		os.system("convert -sampling-factor 4:2:0 -quality %d %s temp.jpg"%(q, f)) 
		os.system("cp temp.jpg %s/q_%d/%d.jpg"%(root, q, i))
		psnr += float(commands.getstatusoutput("compare -metric PSNR %s temp.jpg  temp_diff.png"%(f))[1])
		ssim += float(commands.getstatusoutput("pyssim %s temp.jpg"%(f))[1])
	...
	for t1 in thres1:
		...
		for t2 in thres2:
			...
			for i in range(1, N+1):
				...
				commands.getstatusoutput("/opt/libjpeg-turbo-lromp-only/bin/jpegtran -copy none -optimize %s temp.jpg"%(f_out))
				psnr += float(commands.getstatusoutput("compare -metric PSNR %s %s  temp_diff.png"%(origin[i], f_out))[1])
