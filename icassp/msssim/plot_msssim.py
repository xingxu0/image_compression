...
def get_lromp():
	ls = open("lromp_msssim.out").readlines()
	x = []
	y1 = []
	y2 = []
	for l in ls:
...
def get_raw():
	ls = open("icassp_fromraw_msssim.out").readlines()
	x = []
	y1 = []
	y2 = []
	flag = False
...
def get_75():
	ls = open("icassp_from_q_msssim.out").readlines()
	x = []
	y1 = []
	y2 = []
	for l in ls:
...
ax2.set_ylabel("MS-SSIM", fontsize = label_font_size)
ax2.set_xlabel("Size", fontsize = label_font_size)
ax2.plot(x_lromp, lromp_ssim, "-+")
ax2.plot(x_raw, raw_ssim, "-x")
ax2.plot(x_75, _75_ssim, "-o")
#ax.set_title("Saving=%.3f, Decoding=%.3fMS"%(saving, decoding))
ax2.grid()
ax2.set_xlim([120000,220000])
ax2.set_ylim([0.990,0.998])
#legend = []
...
savefig("plot_msssim.png")#final_%d_.png"%(tested_file_size))
savefig("plot_msssim.eps")#%(tested_file_size))
