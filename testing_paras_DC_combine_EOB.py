...
def calc_gain(comp, dep1_s, dep2_s):
	...
	for f in files:
		...
		for ii in range(len(block_t)):
			...
			z = 1
			if lib.all_zero_block(b):
				z = 0			
			oc_dc_t[lib.get_previous_blocks_coef_for_DC(block_t, ii)][(b[0]<<1) + z] += 1
			oc_dc_opt[b[0]] += 1
			r = 0
			pos = 1
		...
		for x in range(12):
			...
			if comp == "0":
				total_baseline_dc += oc_dc_opt[x]*lib.bits_dc_luminance[x]
			else:
				total_baseline_dc += oc_dc_opt[x]*lib.bits_dc_chrominance[x]
	...
	total_gain = sum(j) - sum(yy)
	gain_dc = 0
	jdc = total_baseline_dc
	our_dc = 0
	for i in range(21):
		for x in oc_dc_t[i]:
			if oc_dc_t[i][x] != 0:
				our_dc += oc_dc_t[i][x]*co_dc[i][x]
	gain_dc = jdc - our_dc
	...
	lib.fprint("JPEG baseline DC  symbol bits:" + str(jdc) + "\tour symbol bits:" + str(our_dc) + "\tdifference:" +str(gain_dc))
	
	lib.fprint("\nJPEG optimize run length bits:" + str(total_opt) + "\tour run length bits:" + str(sum(yy)) + "\tdifference:" + str(total_opt-sum(yy)))
	lib.fprint("JPEG optimize DC  symbol bits:" + str(total_opt_dc) + "\tour symbol bits:" + str(our_dc) + "\tdifference:" +str(total_opt_dc-our_dc))
	...
	if t_dc_s != jdc:
		...
		lib.fprint("ERROR 2 dc symbol not equal!!" + str(t_dc_s) + " " + str(jdc))
	...
	lib.fprint("DC        : gaining bits:" + str(total_opt_dc-our_dc) + "\ttotal bits in file:" + str(t_total_bits_opt))
	lib.fprint("gaining " + str(total_gain+total_opt-sum(j)+total_opt_dc-our_dc) + " bits (" + str((total_gain+total_opt-sum(j)+total_opt_dc-our_dc)*100.0/t_total_bits_opt)+"%)")
