...
def get_candidates_size(img_folder):
	...
	for f in glob.glob("%s/*.jpg"%(img_folder)):
		...
		c = commands.getstatusoutput("cp %s %s/temp.jpg"%(f, img_folder))
		c = commands.getstatusoutput("./packJPG -o %s/temp.jpg"%(img_folder))
		total_pjg_size += os.path.getsize(img_folder + "/temp.pjg")
		os.system("rm " + img_folder + "/temp.pjg")
...
std,opt,ari,pro,moz,pjg = get_candidates_size("%s/test"%(root))
...
printf(f_out, "our %d, std %d, opt %d, ari %d, pro %d, moz %d, pjg %d"%(total_encoded_size, std, opt, ari, pro, moz, pjg))
f_out.close()
