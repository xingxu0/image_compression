...
void zerooff(JCOEF* b, signed char* huff_code) {
    int i=0;
      ...
      int ci, previous_block_state_t * previous_block_state)
{
  ...
  if (ci)
	  zerooff(block, chro_huff_code);
  else
	  zerooff(block, lumi_huff_code);
...
encode_one_block (working_state * state, JCOEFPTR block, int last_dc_val,
                  c_derived_tbl *dctbl, c_derived_tbl *actbl, int ci, boolean lromp)
{
  if (lromp) {
  if (ci)
	  zerooff(block, chro_huff_code);
  ...
  for (blkn = 0; blkn < cinfo->blocks_in_MCU; blkn++) {
    ...
    if (cinfo->private_option == 1) { // Xing: for our special encoding
      ...
      if (! encode_one_block(&state,
                             ...
                             entropy->ac_derived_tbls[compptr->ac_tbl_no], ci, cinfo->private_option==5))
        return FALSE;
    }
    /* Update last_dc_val */
